...
import { Baby, Users, Menu, X, Activity, ShieldAlert, TrendingUp, Clock, Mail, Info } from 'lucide-react';
...
export default function EndoCalc() {
  ...
  // SPL State (Independent)
  const [splValue, setSplValue] = useState(4.5);
  const [splAge, setSplAge] = useState(10);

  const { targetHeight, thSDS } = calculateTargetHeightData(fatherHt, motherHt, sex);
  
  // Safe Calculations
  const heightRes = useMemo(() => {
    try { return calculateHeightSDS(height, age, sex); } 
    catch (e) { return { sds: 0, standard: 'WHO' as const }; }
  }, [height, age, sex]);

  const weightRes = useMemo(() => {
    try { return calculateWeightSDS(weight, age, sex); }
    catch (e) { return { sds: 0, standard: 'WHO' as const }; }
  }, [weight, age, sex]);
  ...
  const splRes = useMemo(() => calculateSPLSDS(splValue, splAge), [splValue, splAge]);
  ...
  return (
    <div className="min-h-screen bg-[#05070a] p-4 md:p-8 flex flex-col items-center overflow-x-hidden text-white">
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <>
            ...
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className="fixed right-0 top-0 bottom-0 w-72 bg-[#0a0c10] border-l border-white/10 z-50 p-8 flex flex-col shadow-2xl">
              <div className="flex justify-between items-center mb-12">
                <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/40">Clinical Suite</h2>
                <button onClick={() => setIsMenuOpen(false)} className="p-2 hover:bg-white/5 rounded-full transition-colors"><X className="w-5 h-5 text-white/60" /></button>
              </div>
              ...
              <div className="pt-8 border-t border-white/5 space-y-4">
                ...
                <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
                  <p className="text-[8px] text-white/20 uppercase tracking-widest font-black leading-relaxed text-center">EndoCalc Alpha v1.4.5</p>
                </div>
              </div>
            </motion.div>
          </>
      ...
      <div className="w-full max-w-5xl flex justify-between items-center mb-8 relative z-30">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            <span className="px-2 py-0.5 bg-amber-400/10 border border-amber-400/20 rounded text-[7px] text-amber-400 font-black uppercase tracking-widest tracking-tighter">Alpha Testing</span>
          </div>
        </div>
        <button onClick={() => setIsMenuOpen(true)} className="p-3 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 transition-all active:scale-95"><Menu className="w-6 h-6 text-cyan-400" /></button>
      </div>

      <AnimatePresence mode="wait">
        {activeModule === 'auxology' ? (
          <motion.div key="auxology" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-5 gap-8">
            <div className="lg:col-span-2 space-y-6">
              <section className="p-8 backdrop-blur-3xl bg-white/[0.02] border border-white/10 rounded-[2.5rem] shadow-2xl">
                <div className="flex items-center gap-3 mb-8 text-cyan-400"><Baby className="w-4 h-4" /><h2 className="text-[10px] font-bold uppercase tracking-widest opacity-60">Patient Details</h2></div>
                <div className="space-y-8">
                  <div className="flex gap-4">
                    <button onClick={() => setSex('male')} className={`flex-1 py-4 rounded-xl border-2 transition-all text-[10px] font-bold uppercase tracking-widest ${sex === 'male' ? 'border-cyan-400 bg-cyan-400/10 text-cyan-400' : 'border-white/5 text-white/20'}`}>Boy</button>
                    <button onClick={() => setSex('female')} className={`flex-1 py-4 rounded-xl border-2 transition-all text-[10px] font-bold uppercase tracking-widest ${sex === 'female' ? 'border-amber-400 bg-amber-400/10 text-amber-400' : 'border-white/5 text-white/20'}`}>Girl</button>
                  </div>
                  <div>
                    <div className="flex justify-between items-center mb-4"><label className="text-[10px] text-white/30 uppercase font-bold">Age (Years)</label><input type="number" step="0.01" value={age} onChange={(e) => setAge(parseFloat(e.target.value) || 0)} className="bg-white/5 border border-white/10 rounded-lg text-cyan-400 font-mono text-sm font-bold text-center w-16 py-1 focus:border-cyan-400 focus:outline-none" /></div>
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-4"><label className="text-[10px] text-white/30 uppercase font-bold">Height (cm)</label><input type="number" step="0.1" value={height} onChange={(e) => setHeight(parseFloat(e.target.value) || 0)} className="bg-white/5 border border-white/10 rounded-lg text-cyan-400 font-mono text-sm font-bold text-center w-16 py-1 focus:border-cyan-400 focus:outline-none" /></div>
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-4"><label className="text-[10px] text-white/30 uppercase font-bold">Weight (kg)</label><input type="number" step="0.1" value={weight} onChange={(e) => setWeight(parseFloat(e.target.value) || 0)} className="bg-white/5 border border-white/10 rounded-lg text-cyan-400 font-mono text-sm font-bold text-center w-16 py-1 focus:border-cyan-400 focus:outline-none" /></div>
              ...
              <section className="p-8 backdrop-blur-3xl bg-white/[0.02] border border-white/10 rounded-[2.5rem] shadow-2xl">
                <div className="flex items-center gap-3 mb-6 text-cyan-400"><Users className="w-4 h-4" /><h2 className="text-[10px] font-bold uppercase tracking-widest opacity-60">Parental Heights</h2></div>
                <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-white/5 border border-white/10 rounded-2xl"><label className="text-[8px] text-white/30 uppercase font-bold mb-2 block text-center">Father (cm)</label><input type="number" value={fatherHt} onChange={(e) => setFatherHt(parseInt(e.target.value) || 0)} className="bg-transparent text-white font-mono text-xs font-bold w-full text-center focus:outline-none" /></div>
                    <div className="p-4 bg-white/5 border border-white/10 rounded-2xl"><label className="text-[8px] text-white/30 uppercase font-bold mb-2 block text-center">Mother (cm)</label><input type="number" value={motherHt} onChange={(e) => setMotherHt(parseInt(e.target.value) || 0)} className="bg-transparent text-white font-mono text-xs font-bold w-full text-center focus:outline-none" /></div>
                </div>
              </section>
            </div>
            ...
            <div className="lg:col-span-3 space-y-6">
              ...
              <div className="p-8 bg-white/5 border border-white/10 rounded-[2.5rem] shadow-2xl flex items-center justify-between">
          ...
          <motion.div key="spl" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-8">
            <section className="p-8 backdrop-blur-3xl bg-white/[0.02] border border-white/10 rounded-[2.5rem] shadow-2xl">
                <div className="flex items-center gap-3 mb-8 text-cyan-400"><Activity className="w-5 h-5" /><h2 className="text-sm font-bold uppercase tracking-widest opacity-60">SPL Assessment</h2></div>
                <div className="space-y-8">
                    <div>
                        <div className="flex justify-between items-center mb-4"><label className="text-[10px] text-white/30 uppercase font-bold">Assessment Age (Years)</label><input type="number" step="0.1" value={splAge} onChange={(e) => setSplAge(parseFloat(e.target.value) || 0)} className="bg-white/5 border border-white/10 rounded-lg text-cyan-400 font-mono text-sm font-bold text-center w-16 py-1 focus:border-cyan-400 focus:outline-none" /></div>
                        <input type="range" min="0" max="18" step="0.1" value={splAge} onChange={(e) => setSplAge(parseFloat(e.target.value))} className="w-full h-1 bg-white/10 rounded-lg appearance-none cursor-pointer accent-cyan-400" />
                    </div>
                    <div>
                        <div className="flex justify-between items-center mb-4"><label className="text-[10px] text-white/30 uppercase font-bold">Measured SPL (cm)</label><input type="number" step="0.1" value={splValue} onChange={(e) => setSplValue(parseFloat(e.target.value) || 0)} className="bg-white/5 border border-white/10 rounded-lg text-cyan-400 font-mono text-sm font-bold text-center w-16 py-1 focus:border-cyan-400 focus:outline-none" /></div>
                        <input type="range" min="1" max="15" step="0.1" value={splValue} onChange={(e) => setSplValue(parseFloat(e.target.value))} className="w-full h-1 bg-white/10 rounded-lg appearance-none cursor-pointer accent-cyan-400" />
                    </div>
                    <div className="p-6 bg-white/5 border border-white/10 rounded-2xl"><p className="text-[8px] text-white/20 uppercase font-bold mb-2">Cohort Identification</p><p className="text-white text-[10px] font-bold uppercase tracking-wider">{splRes?.label || 'Unknown Age Group'}</p></div>
                </div>
            </section>
